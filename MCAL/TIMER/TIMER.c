...
STD_ReturnType TIMER0_Init(void)
{
     ...
     * 5. Return E_OK.
     */
    TIMER0_REG_TCCR0 = (1 << 3) | (0 << 6);   /* WGM01 = 1, WGM00  = 0 */
    
    //TIMER0_REG_TCCR0 &= ~(7 << 0); /* CS02:0 = 000 * the delay function will set the prescaler */
    TIMER0_REG_OCR0 = 124; // Set OCR0 for 1 ms delay
    TIMER0_REG_TCNT0 = 0;
    return E_OK;
}
STD_ReturnType TIMER0_DelayMS(uint16 Copy_u16Milliseconds)
{
     ...
     */
    TIFR_REG |= (1 << 1);                    /* Clear OCF0 */
    TIMER0_REG_TCCR0 |= (1 << 0) | (1 << 1); /* Start clock with prescaler 64 */
    for (uint16 i = 0; i < Copy_u16Milliseconds; i++)
    {
        TIMER_WaitFlag(&TIFR_REG, (1 << 1)); /* Wait for OCF0 */
    }
    TIMER0_REG_TCCR0 &= ~(7 << 0); /* Stop clock */
    return E_OK;
}
...
STD_ReturnType TIMER0_PWM(uint8 Copy_u8DutyPercent)
{
    ...
    TIMER0_REG_TCCR0 |= (1 << 3) | (1 << 6);                  /* WGM01 = 1, WGM00 = 1 */
    TIMER0_REG_TCCR0 |= (1 << 5);                             /* COM01 = 1, COM00 = 0 */
...
STD_ReturnType TIMER0_Stop(void)
{
     ...
     */
    TIMER0_REG_TCCR0 &= ~(7 << 0); /* Stop clock */
    TIMER0_REG_TCCR0 &= ~(3 << 4); /* Clear COM01:COM00 */
    return E_OK;
}
...
static uint16 TIMER_DutyToCompare(uint16 Copy_u16Top, uint8 Copy_u8DutyPercent)
{
    ...
    uint32 temp = ((uint32)Copy_u16Top + 1) * Copy_u8DutyPercent / 100;
    return (uint16)temp;
}
